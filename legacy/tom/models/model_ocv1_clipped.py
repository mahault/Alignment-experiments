...
# turning the model defined into a pymdp agent where model validation is also conducted
...
def generate_obs_modalities(layout_info):
    ...
    # add other_location_obs at the end, after all objects
    obsmodalities["other_location_obs"] = {
        "size": layout_info["nonwall_locations"],
...
def generate_state_factors(layout_info):
    ...
    carrying_depends_on = ["self_carrying_state", "self_facinglocation_state"]
    ...
    statefactors = {
        ...
        "self_facinglocation_state": {
            "size": layout_info["all_locations"],
            "depends_on": ["self_facinglocation_state", "self_location_state", "other_location_state"],
            "controlled_by": ["actions"],
        },
        "self_carrying_state": {
            "elements": ["nothing", "onion"],
    ...
    # add other_location_state at the end, after all objects
    statefactors["other_location_state"] = {
        "size": layout_info["nonwall_locations"],
        "depends_on": ["other_location_state"],
...
def OvercookedModel(env_layout):
    ...
    model.A["other_location_obs"].data = jnp.eye(layout_info["nonwall_locations"])
    ...
    # agents enter deadlock when agents occupy the same location
    for loc in range(layout_info["nonwall_locations"]):
        # zero out all transitions when self and other are in same location
        model.B["self_location_state"][:, loc, loc, :] = 0.0
        # set transition to stay in same location
        model.B["self_location_state"][loc, loc, loc, :] = 1.0

    # # agents cannot move to locations currently occupied by the other agent
    # # this prevents location swaps: if other is at location X, self cannot move to X 
    # # TODO currently, this happens regardless of whether other is trying to move to self's current location - fix this
    # for current_self_loc in range(layout_info["nonwall_locations"]):
    #     for other_loc in range(layout_info["nonwall_locations"]):
    #         if current_self_loc != other_loc:  # skip if already at same location (handled above)
    #             # zero out all transitions that would move self to other's current location
    #             model.B["self_location_state"][other_loc, current_self_loc, other_loc, :] = 0.0
    #             # set transition to stay in current location instead
    #             model.B["self_location_state"][current_self_loc, current_self_loc, other_loc, :] = 1.0
    ...
    for agent_nonwall_idx in range(layout_info["nonwall_locations"]):
        ...
        for other_nonwall_idx in range(layout_info["nonwall_locations"]):
            for current_facing_loc in range(layout_info["all_locations"]):
                for action, (dr, dc) in action_to_direction.items():
                    if action == "stay" or action == "interact":
                        ...
                        model.B["self_facinglocation_state"][current_facing_loc, current_facing_loc, agent_nonwall_idx, other_nonwall_idx, action] = 1.0
                    else:
                        # calculate the agent's new position after taking the action
                        new_row, new_col = agent_row + dr, agent_col + dc
                        
                        # check if the new position is valid (within bounds and not a wall)
                        if (0 <= new_row < env_layout["height"] and 0 <= new_col < env_layout["width"]):
                            new_all_loc = int(coords_to_pos(new_row, new_col))
                            if new_all_loc not in layout_info["wall_set"]:
                                # agent successfully moves, so calculate facing location from new position
                                facing_row, facing_col = new_row + dr, new_col + dc
                            else:
                                # agent hits a wall, stays in current position
                                facing_row, facing_col = agent_row + dr, agent_col + dc
                        else:
                            # agent tries to move out of bounds, stays in current position
                            facing_row, facing_col = agent_row + dr, agent_col + dc
                        
                        # set the facing location
                        if (0 <= facing_row < env_layout["height"] and 0 <= facing_col < env_layout["width"]):
                            ...
                            model.B["self_facinglocation_state"][next_facing_loc, current_facing_loc, agent_nonwall_idx, other_nonwall_idx, action] = 1.0
                        else:
                            # trying to face out of bounds, keep current facing location
                            model.B["self_facinglocation_state"][current_facing_loc, current_facing_loc, agent_nonwall_idx, other_nonwall_idx, action] = 1.0

    # agents in deadlock cannot change facing location when they occupy the same location
    for loc in range(layout_info["nonwall_locations"]):
        # zero out all facing transitions when self and other are in same location
        model.B["self_facinglocation_state"][:, :, loc, loc, :] = 0.0
        # set transition to keep current facing location for all actions
        for facing_loc in range(layout_info["all_locations"]):
            model.B["self_facinglocation_state"][facing_loc, facing_loc, loc, loc, :] = 1.0
    ...
    for action in ["up", "down", "right", "left", "stay"]:
        for carrying_state in range(model.B["self_carrying_state"].data.shape[0]):
            ...
            indices = [carrying_state, carrying_state] + [slice(None)] * (1 + total_object_count) + [action]
    ...
    for carrying_state in range(model.B["self_carrying_state"].data.shape[0]):
        ...
        indices = [carrying_state, carrying_state] + [slice(None)] * (1 + total_object_count) + ["interact"]
        model.B["self_carrying_state"][tuple(indices)] = 1.0
    ...
    if onion_pile_count > 0:
        for onion_pile_idx in range(onion_pile_count):
            for facing_loc in range(layout_info["all_locations"]):
                ...
                indices = ["onion", "nothing", facing_loc] + pile_states + ["interact"]
                model.B["self_carrying_state"][tuple(indices)] = 1.0
                ...
                indices_clear = ["nothing", "nothing", facing_loc] + pile_states + ["interact"]
    ...
    #             indices = ["plate_empty", "nothing", facing_loc] + pile_states + ["interact"]
    #             model.B["self_carrying_state"][tuple(indices)] = 1.0
    ...
    ######### B TENSOR FOR OTHER_LOCATION_STATE #########
    # compute valid transitions based on grid layout
    valid_transitions = {}
    for from_nonwall_idx in range(layout_info["nonwall_locations"]):
        from_all_loc = int(layout_info["nonwall_to_all"][from_nonwall_idx])
        from_row, from_col = int(pos_to_coords(from_all_loc)[0]), int(pos_to_coords(from_all_loc)[1])
        
        valid_transitions[from_nonwall_idx] = []
        # check all adjacent locations (up, down, left, right, stay)
        for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1), (0, 0)]:
            to_row, to_col = from_row + dr, from_col + dc
            
            # check if target is valid (witin bounds and not a wall)
            if (0 <= to_row < env_layout["height"] and 0 <= to_col < env_layout["width"]):
                to_all_loc = int(coords_to_pos(to_row, to_col))
                if to_all_loc not in layout_info["wall_set"]:
                    to_nonwall_idx = int(layout_info["all_to_nonwall"][to_all_loc])
                    if to_nonwall_idx not in valid_transitions[from_nonwall_idx]:
                        valid_transitions[from_nonwall_idx].append(to_nonwall_idx)

    # set uniform probability for valid transitions (including staying in place)
    for from_nonwall_idx, to_nonwall_indices in valid_transitions.items():
        if len(to_nonwall_indices) > 0:
            probability = 1.0 / len(to_nonwall_indices)
            for to_nonwall_idx in to_nonwall_indices:
                model.B["other_location_state"][to_nonwall_idx, from_nonwall_idx, "noop"] = probability

    '''
    BUILDING THE C TENSOR.     
    '''
    model.C["self_facinglocation_obs"][2] = 1.0
