...
def rollout(
    ...
    def step_fn(carry, t):
        ...
        observation_next, env_state_next = env.step(
            rng_key=keys[2], env_state=env_state, actions=action_next
        )  # step environment forward with chosen action
    ...
    # only transpose arrays that have at least 2 dimensions 
    def transpose_info(x):
        if hasattr(x, 'ndim') and x.ndim >= 2:
            return x.transpose((1, 0) + tuple(range(2, x.ndim)))
        else:
            return x
    
    info = jtu.tree_map(transpose_info, info)  # transpose to have timesteps as first dimension
