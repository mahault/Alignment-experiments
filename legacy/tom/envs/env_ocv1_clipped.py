...
from jaxmarl.environments.overcooked.common import OBJECT_TO_INDEX, DIR_TO_VEC
...
def extract_agent_obsmodalities(agent_obs, obsmodality, coord_mapping=None, layout=None, goal_delivered_state=0):
    if obsmodality == 'location':
    ...
    elif obsmodality == 'other_location':
        # Get other agent's position (the next agent in sequence)
        pos_channel = agent_obs[:, :, CHANNEL_MAP['agent_1_pos']]
        coords = jnp.where(pos_channel > 0, size=1, fill_value=-1)
        x, y = coords[0][0], coords[1][0]
        ...
        # Convert to nonwall location index
        wall_set = set(layout["wall_idx"].tolist())
        nonwall_locations = jnp.array([loc for loc in range(len(coord_mapping)) if loc not in wall_set])
        matches = (nonwall_locations == all_location_idx)
        nonwall_idx = jnp.where(matches, jnp.arange(len(nonwall_locations)), -1).max()
        ...
        carrying_elements = ["nothing", "onion"]
        ...
        carrying_state = jnp.where(carrying_onion, onion_idx, nothing_idx)
        ...
        if obsmodality.endswith(('0', '1', '2', '3', '4')):  # currently supports up to 5 objects
            ...
            valid_layout_keys = ["onion_pile_idx"]
            # valid_layout_keys = ["onion_pile_idx", "plate_pile_idx", "pot_idx", "goal_idx"]
...
def generate_obsmodalities_from_layout(layout):
    ...
    obsmodalities = ["location", "facinglocation", "self_carrying"]
    # obsmodalities = ["location", "facinglocation", "self_carrying", "pot", "goal_delivered"]

    # add observations for each object type found in layout (matching the model)
    object_keys = ["onion_pile_idx"]
    
    for layout_key in object_keys:
        if layout_key in layout and len(layout[layout_key]) > 0:
            for i in range(len(layout[layout_key])):
                obsmodalities.append(f"{layout_key}{i}")
    
    # add other_location at the end, after all objects
    obsmodalities.append("other_location")
...
class OvercookedV1Env(Module):
    ...
    initiate_facingdir: list = field(static=True)

    def __init__(self, num_agents, layout, max_steps, initiate_inventory, initiate_facingdir=None):
        ...
        self.initiate_facingdir = initiate_facingdir
    ...
    def reset(self, key):
        ...
        # modifying the JaxMARL environment state if there is an initial inventory or facing direction
        if self.initiate_inventory is not None or self.initiate_facingdir is not None:
            ...
            if self.initiate_inventory is not None:
                ...
                new_state = new_state.replace(agent_inv=inventory_indices)
            
            # set facing directions if specified
            if self.initiate_facingdir is not None:
                # convert facing direction indices to direction vectors
                facing_indices = jnp.array(self.initiate_facingdir)
                facing_vectors = DIR_TO_VEC[facing_indices]
                # updating the state with new facing directions
                new_state = new_state.replace(
                    agent_dir_idx=facing_indices,
                    agent_dir=facing_vectors
                )
            
            # recalculating observations with new state
