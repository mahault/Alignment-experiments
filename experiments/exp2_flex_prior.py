...
def init_env_and_agents(config: Exp2Config):
    ...
    # Build ToM agents with F-aware prior
    ...
    agent_config = AgentConfig(
        ...
        kappa_prior=config.kappa_prior,  # Exp 2: USE flexibility prior
    ...
    LOGGER.info(f"Created {len(agents)} ToM agents with F-aware prior")
    LOGGER.info(f"  Policies per agent: {len(focal_agent.policies)}")
    LOGGER.info(f"  Horizon: {config.horizon}, Gamma: {config.gamma}")
    LOGGER.info(f"  Alpha (empathy): {config.alpha_empathy}, Kappa (F-prior): {config.kappa_prior}")
    LOGGER.info(f"  Beta (joint flex): {config.beta_joint_flex}")
